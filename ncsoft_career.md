...
> ### Authentication
> #### External IdP
> * (inhouse)런처나 클라이언트가 각 플랫폼 별 로그인 방식에 따라 로그인 후 OAuth 토큰 획득
...
>     + (계정 툴과 계정 관리 서버는 다른 팀원 분이 제작 및 관리)
> * 생성된 내부 계정 발급과 동시에 계정(id/pw)을 토큰화하여 토큰(내부 계정 토큰)도 같이 발급 가능 (파일 형태)
> * 발급한 내부 계정 토큰(또는 내부 계정)은 클라이언트에서 파일로 로드
> * 클라이언트가 내부 계정 토큰을 인증 서버에 연결 후 전달
> * 인증 서버는 내부 계정 토큰을 해석 후 인증 수행
