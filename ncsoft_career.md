...
## Lobby
* (원래 로비 서버의 담당자가 따로 있었으나 부서 이동으로 담당자 부재와 이슈와 함께 출시를 위한 scale out이 가능한 구조로의 개선이 지상과제가 되면서 작성자로 담당자가 변경된 후 이미 있는 기능을 보완하거나 추가로 작업한 내용들을 정리)
### 작업 내용
#### Login
* 자체 세션 서버(C/C++)에서 유저 세션 관리하는 부분을 redis++을 사용하여 메모리 데이터베이스와 연동하도록 수정
* 위 작업으로 인한 유저 로그인 처리 로직 대부분 수정
#### Nickname * Character
* 인게임에서 사용할 닉네임과 캐릭터 관련 준비 작업(Preparation)
#### Character
