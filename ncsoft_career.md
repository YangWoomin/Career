...
* 안그래도 자체 구축한 K8s On-Premise를 사용하는데 문제가 있었어서 NCKUBE를 통해 서버를 배포하도록 migration 작업을 시도함
  + K8s On-Premise 사용 문제는 서버 pod간 연결이 매우 자주 간헐적으로 단절되는 현상이 있었음
  + K8s 클러스터 자체에서 발생하는 문제가 아닌 한쪽 endpoint에서 강제로 연결이 종료되는 현상
    + 그렇다고 해서 프로그래밍적인 문제도 아니었음 (K8s 클러스터 환경에서만 발생함)
  + 의심되는 원인으로는, K8s 클러스터 노드 중에 리눅스 노드들이 있는 네트워크 대역과 윈도우 노드들이 있는 네트워크 대역이 달랐고 이 둘 사이에서만 연결이 끊어지는 것을 파악했고 사내 보안 정책이 가장 유력해 보였음
  + 사내 네트워크 보안팀에 문의한 결과 
### Ansible (GStar)
### Trouble shooting
<details>
