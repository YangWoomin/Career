...
> #### Redis의 "KEYS"를 사용하지 않고 redirect key를 사용 (Trouble shooting보다는 노하우?)
...
> #### Redis++의 Sync -> Async 전환, 그리고 개선해야 할 일
> * Redis에 CRUD 작업을 처음부터 Sync 모드로 수행했더니 문제가 생겨서 Async 모드로 변환한 것은 아니고 미리 throughput이 나오지 않을 것을 예상하여 개발 도중에 Sync -> Async 모드로 변경
>   + 로비 서버의 모든 스레드가 Redis CRUD 작업을 위해 blocking 되어 있다면 
