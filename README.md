...
안녕하세요. 기술과 경험에 항상 목말라있는 개발자 양우민 입니다.

이 인사말을 쓰면서 저를 형용하는 "기술과 경험에 항상 목말라있는 개발자" 라는 문구를 붙이면서 잠깐 고민을 했습니다.

나를 표현하기에 이보다 더 좋은 문구는 없을 것 같은데 5년 뒤, 10년 뒤, 20년 뒤에도 저 문구가 계속 유효할까? 라는 생각이 들었습니다.

자신의 총 경력이나 포트폴리오를 소개하는 메인 페이지에 적는 인사말은 자신의 변하지 않는 강력한 장점을 잘 나타내는게 중요하다고 생각하기 때문입니다.

이렇게 짧은 고민을 했지만, 지금까지 살아온 인생을 뒤돌아 봤을 때 앞으로도 바뀌지 않을 것이라는 확신이 들었습니다.

이유는 제가 근본적으로 욕심이 많아서 입니다.

공동체에서 없어서는 안 될 구성원으로서 살아가는 것이 제 인생의 가장 큰 낙인데,

없어서는 안 될 구성원이 되려면 계속 자신을 채찍질하며 성장시켜야 하고 이것이 본능적으로 자기 자신을 채우려는 욕심으로 나타나는 것 같습니다.

얼마나 욕심이 많길래? 라고 생각들지 않으신가요?

아래에 제가 지금까지 해 온 것들을 정리해두었습니다.

C/C++ 서버 개발부터 웹 프론트엔드, 툴, CI/CD, 인프라... 회사에서 하던 것들 외에 자기 계발까지.
...
##### 성과
* 서버 기반 시스템 및 외부 연동 지원
* 서버 운영을 위한 인프라 및 CI/CD 지원
* 2023 지스타 현장 LLL 서비스 구축 및 운영

<details>
<summary>조금 풀어쓴 경력사항 (클릭하여 펼치기)</summary>

> 2019년부터 5년 이상을 LLL 프로젝트에 몸담았으며 주로 인증(nano, steam)/로비 서버 개발과 서버의 외부 라이브러리(redis, kafka) 연동을 위한 기반 시스템 개발, 서버 빌드 및 배포, 인프라 쪽을 맡았습니다.
> 게임 컨텐츠 개발을 제외한 나머지 일들을 주로 해오다 보니 서버의 기반 작업/외부 연동이나 오픈소스, 인프라 중심의 많은 것들을 경험해 보았습니다. 인증 서버에서 사내 계정으로 게임 로그인을 지원하기 위한 nano 인증 연동, 스팀 계정으로 플레이하기 위한 스팀 API 연동 등을 작업했으며, 로비 서버의 scale out이 가능한 형태로의 개선을 위한 유저 세션 저장소인 redis 연동, 서버 간 연결 복잡도를 낮추고 메시지 송수신 구현의 의존성을 낮추기 위한 메시지 큐(kafka) 연동 등이 최근에 수행한 작업입니다. 그 외 Make/CMake를 사용한 서버 빌드, 팀시티를 사용한 서버 CI 등을 지원하였습니다. 인프라 쪽은 서버 배포 및 실행을 위한 docker, k8s, 그리고 ansible 등을 사용했으며 서버의 지표와 로그 수집을 위한 InfluxDB + Grafana 그리고 Elasticsearch + Kibana + Fluentd 등을 운영하였습니다.
> 
> 주 언어는 아무래도 지금까지 작업해 온 서버들의 구현 언어인 C/C++이며 modern C++도 많은 관심을 가지고 있습니다. Golang이나 C#, Python 등의 언어도 서버 프로토타입 개발이나 여러 툴에도 사용해 본 경험이 있어서 다양한 언어들을 사용하는 것을 선호합니다. 해보고 싶은 일들은 지금껏 해온 일들과 비슷한 플랫폼 연동 개발이나 기반 시스템 개발에 관심이 있으며 컨텐츠도 해보고 싶은 생각이 있습니다. 특히 기반 시스템 개발에 관심이 있어서 이번에 준비한 포트폴리오를 지금까지 일하며 습득한 내용을 기반으로 준비했습니다. 
> 
> 경력 기술과 관련하여 더 자세한 내용은 https://github.com/YangWoomin/Career/blob/master/career_y_ncsoft.md 페이지에서 확인하실 수 있습니다.
...
##### 성과
* 프리스타일 풋볼 프로젝트 적자 -> 흑자 전환 및 유지
* 프리스타일 풋볼 한국/중국 일매출 경신
* 고질적인 서버 크래시 문제 해결

<details>
<summary>조금 풀어쓴 경력사항 (클릭하여 펼치기)</summary>

> 2017년부터 2년 동안 몸담았던 프로젝트입니다. 신입으로 처음 입사한 회사이며 입사한 지 6개월 만에 프로젝트의 서버 파트를 홀로 담당하게 되었습니다. 이유는 저무는 프로젝트이기도 했고 지속적인 적자 프로젝트이다 보니 인력 재배치로 인해 저 혼자 남게 되었었습니다. 그렇게 클라이언트 2명과 기획자 1명, 서버 1명으로 이 프로젝트를 담당하게 되었습니다. 인력 감축으로 적자 폭은 줄었으나 그저 IP 유지를 위한 명목으로 경영진 분들의 큰 관심 대상은 아니어서 간섭이 없었습니다. 하지만 동기인 기획자 동료와 저는 이 프로젝트를 잘 이끌어 보자는 생각이 일치했고 여러 모로 가능한 시도를 해보았습니다. 기획자 동료는 동접과 매출 방어를 위해 매달 끊임 없는 게임 내 여러 이벤트를 제시했으며 저는 군소리 없이 요구하는 것들을 서비스에 잘 적용했습니다. 이로써 적어도 적자만 내는 프로젝트라는 꼬리표를 떼기 시작하였습니다. 여기에 더해 어떤 때는 집에서 아프리카tv로 방송을 보며 우리 게임을 모니터링하는 와중에 유저(스트리머)가 높은 ELO(MMR) 때문에 매칭이 안되는 문제를 겪고 있었고 일부러 ELO를 낮추기 위해 패작하는 것을 목격하였습니다. 이를 해결하고자 기획자 동료에게 모니터링했던 내용을 공유하며 ELO 시즌제를 도입해 보자는 의견을 내었고 흔쾌히 받아들인 기획자 동료는 ELO 시즌제 기획안을 제시해 주었고 저는 그것을 잘 구현해 내었습니다. 이 내용을 서비스에 업데이트하고 얼마 지나지 않아 유저들에게 아주 좋은 반응을 이끌어 냈고 동접은 크게 올라가게 되었습니다. 이때 당시 괜찮은 BM도 같이 나온 터라 중국과 한국 지역에서 일 매출 경신도 이뤄냈었습니다. 이 외에 클랜 시스템 개선 등 크게 눈에 띈 성과는 아니지만 꾸준한 시도들을 해보았고 결과는 대부분 좋았습니다. 그리고 이 게임이 유저들이 느끼기에 죽은 게임처럼 느껴지지 않도록 GM을 통해 전달되는 CS 이슈들을 시간이 나는 대로 바로바로 대응했습니다. CS 문의 중 대부분 자기 아이템이 사라졌다는 유저의 거짓말 CS이지만 진짜로 시스템에 문제가 있어서 그럴 수도 있다는 생각으로 한 번도 간과하지 않고 증거 자료(db/로그 등)와 함께 대응하였습니다. 이러한 부분들을 경영진 분들이 상당히 좋게 평가해 주셔서 매 평가가 최고였지 않나 싶습니다.
> 
> 위 내용은 성과 1번과 2번에 해당하는 내용이며 3번에 해당하는 내용은 https://github.com/YangWoomin/Career/blob/master/career_z_joycity.md 페이지에서 확인하실 수 있습니다.
...
|----------|----------|----------|----------|----------|----------|
| 2019.01 ~ 2024.09 | NCSOFT | 5 | [career_y_ncsoft.md](career_y_ncsoft.md) | [ZilliaxServer](https://github.com/YangWoomin/ZilliaxServer) ([설명서](https://github.com/YangWoomin/Career/blob/master/portfolio_description.md) 참고) | A -> A -> A -> E -> E |
...
* [프로그래머스 제공 100 문제로 완벽 대비 코딩 테스트 합격자 되기 C++편](#프로그래머스-제공-100-문제로-완벽-대비-코딩-테스트-합격자-되기-C편) 문제 풀이
...
운영체제 개념을 다시 한번 정리하기 위해 (거의 다) 읽어본 책
...
Docker와 K8s를 사용한 CD 구축을 위해 읽어본 책
...
### OAuth 2 in Action
![oauth2_in_action](https://github.com/user-attachments/assets/590280ee-af07-4ccb-80f0-da5dd6d09aeb)

인증 서버 구현에 앞서 OAuth 2.0 학습을 위해 읽어본 책 (필요한 부분 위주로 읽음)
