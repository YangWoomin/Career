...
* 프로듀서 서버의 프로듀서 설정을 바꿔가며 처리량 증가/감소시켜 보기 (프로듀서 서버 설정 튜닝)
* 컨슈머 서버(클라이언트 메시지 카운터 및 메시지 수집기)의 컨슈머 개수와 컨슈머 서버 자체 개수, 컨슈머 설정 등을 바꿔가며 처리량 증가시켜 보기
...
![image](https://github.com/user-attachments/assets/ea53aac3-b234-4218-96f8-4b5ab50ad037)
<br/>
클라이언트가 처음 실행시 메시지를 비동기적으로 한꺼번에 밀어 넣으면 프로듀서 서버는 이를 10초간 초당 약 2만 5천 ~ 2만 8천개씩 수신 처리 (프로듀서 서버의 네트워크 스레드는 기본적으로 CPU 스레드 개수만큼 생성, 성능 측정 당시 8개)
<br/>
...
<br/>
프로듀서 서버가 2개의 프로듀서 스레드로 약 2ms 마다 각각 한번에 100개씩 메시지를 캐시 서버로부터 조회해와서 적재하기 때문에 client_message 토픽의 메시지 적재량이 최대 4.68K/s까지 치솟음
<br/>
반면 클라이언트 메시지 카운터는 메시지 소비량이 프로듀서 서버의 적재량보다 현저히 낮은 관계로 message_aggregation 토픽의 메시지 적재량이 client_message 토픽의 메시지 적재량보다 낮음
<br/>
클라이언트 메시지 카운터의 소비량이 낮은 이유는 메시지 1개를 소비한 후 캐시 서버에 동기적으로 호출하여 데이터를 업데이트하고 프로듀싱하는 과정을 트랜잭션으로 묶어서 처리하기 때문에 처리량이 떨어짐
...
<br/>
메시지 수집기의 소비량은 클라이언트 메시지 카운터의 적재량과 비슷하기 때문에 메시지 적재/소비에 대한 랙이 없음
