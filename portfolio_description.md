...
* [소개](#소개)
* [상세 설명](#상세-설명)
* [데이터 신뢰성 테스트](#데이터-신뢰성-테스트)
* [메시지 유실 테스트](#메시지-유실-테스트)
* [성능 테스트](#성능-테스트)
* [더 고민해볼 것들](#더-고민해볼-것들)
...
* 모든 내용을 설명드리기 어려우니 **데이터 신뢰성(Data Reliability)** 위주의 내용을 정리하였습니다.
  + 메모리 데이터베이스(레디스 클러스터, 이하 레디스) 및 메시지 큐(카프카 클러스터, 이하 카프카)를 사용하여 백엔드 서버(C++/Golang)간 데이터 송수신 과정에서 **데이터 유실 및 중복 방지** 처리 (논리적 수준 포함, 인박스/아웃박스 패턴 사용)
  + 레디스 클러스터의 여러 노드에 저장된 데이터에 대해 **결과적 일관성(Eventual Consistency)** 방법으로 데이터 일관성을 보장
...
* **이미지 클릭시 페이지를 제대로 출력하지 않는다면 새로고침 후 다시 클릭해주시길 부탁 드립니다.**
...
* PC
  ...
  + Intel(R) Core(TM) i7-8565U CPU @ 1.80GHz (Total Cores. 4, Total Threads. 8)
  + 16GB RAM
  + 512G SSD
* 클라이언트
  + 클라이언트(소켓) 수 : 10
  + 각 클라이언트는 초당 20개씩 메시지를 프로듀서 서버에게 전송
* 프로듀서 서버
  + 1대
  + 하나의 서버의 네트워크 스레드는 하드웨어 스레드 개수
  + 하나의 서버에서 프로듀싱하는 스레드는 2개
  + 각각 스레드는 캐시 서버로부터 100개(기본값)씩 메시지를 로드해서 메시지 큐에 프로듀싱 수행
  + acks=all
...
* 클라이언트 메시지 카운터 및 메시지 수집기
  + 각각 1대
  + 하나의 서버(프로세스)에서 프로듀싱 및 컨슈밍하는 스레드는 각각 3개
  + 클라이언트 메시지 카운터만 프로듀서에서 트랜잭션 사용
    + 메시지 100개씩 1개 트랜잭션으로 커밋
    + 2초(기본값) 동안 추가로 소비할 메시지가 없으면 커밋
...
> 컨슈머(클라이언트 메시지 카운터, 메시지 수집기)에서도 연결이 단절되었다는 로그 확인
> 
> #### 3. 전반적인 작업 흐름은 우아한 브로커 재시작과 크게 다르지 않음
> 
...
> <br/>
> 강제 종료된 브로커에서 데이터 유실이 발생하더라도 REPLICATION FACTOR(ISR)를 3으로 유지하고 있었기 때문에 온전히 보존되던 다른 브로커의 데이터로부터 복구할 수 있음
...
* 컨슈머 서버(클라이언트 메시지 카운터 및 메시지 수집기)의 컨슈머 서버 개수, 컨슈머 설정 등을 바꿔가며 처리량 증가시켜 보기
...
#### 초당 프로듀서 서버가 클라이언트로부터 받는 메시지 개수
![image](https://github.com/user-attachments/assets/c9439ace-5856-45af-84a6-596d5a5bb200)
<br/>
각 클라이언트가 초당 20개의 메시지를, 총 50개의 클라이언트가 전송하므로 프로듀서 서버는 초당 약 1000개씩의 메시지를 수신 (약 4분동안 수신)
<br/>
프로듀서 서버는 클라이언트로부터 수신한 메시지를 곧바로 캐시 서버에 저장 (비동기)

#### 초당 프로듀싱 메시지 개수
![image](https://github.com/user-attachments/assets/68895325-4ad3-434a-a1c2-12dab225b349)
<br/>
프로듀서 서버가 2개의 스레드로 캐시 서버에 저장된 메시지를 메시지 큐에 밀어 넣다 보니 프로듀서 서버가 초당 1000개씩 메시지를 수신하는 것과 비슷하게 메시지 큐에 프로듀싱됨 (평균 약 900개씩 적재)
<br/>
클라이언트 메시지 카운터는 1개의 서버에서 3개씩의 프로듀서/컨슈머로 메시지를 컨슈밍하고, 캐시 서버에 업데이트하고, 다시 오프셋 커밋과 메시지 프로듀싱을 트랜잭션으로 묶어서 처리하다보니 초당 약 600개씩의 메시지를 프로듀싱함

#### 초당 컨슈밍 메시지 개수
![image](https://github.com/user-attachments/assets/e44a6295-d6b2-4647-94e3-cf89679afda2)
<br/>
클라이언트 메시지 카운터는 앞서 설명한 것처럼 메시지를 초당 약 600개씩 컨슈밍, 업데이트, 프로듀싱을 수행함
<br/>
메시지 수집기는 클라이언트 메시지 카운터와 동일한 조건의 프로듀서/컨슈머 개수와 거의 동일한 로직 처리를 수행하지만 트랜잭션 처리를 하지 않으므로 보다 가벼워서 클라이언트 메시지 카운터가 프로듀싱하는 것에 맞춰서 컨슈밍함 (초당 약 600개씩 처리)

#### 컨슈머 그룹 랙 측정
![image](https://github.com/user-attachments/assets/9f061292-eb7d-4b8b-9038-0f0449ef9b9a)
<br/>
프로듀서 서버는 초당 약 900개씩 메시지를 프로듀싱하고 클라이언트 메시지 카운터는 초당 약 600개씩 메시지를 컨슈밍하므로 이 차이가 점점 쌓여서 최대 74.6K개까지 차이가 벌어짐
<br/>
반면 메시지 수집기는 클라이언트 메시지 카운터의 프로듀싱하는 양에 맞춰서 컨슈밍하기 때문에 랙이 없음

### 클라이언트 메시지 카운터 & 메시지 수집기 확장 및 설정 튜닝

#### 기본 테스트에서 확장 및 설정 튜닝한 내용 (클라이언트수 50 유지)
* 캐시 서버 분리
  + 기존에는 3+3개의 레디스 클러스터를 프로듀서 서버, 클라이언트 메시지 카운터, 메시지 수집기가 모두 사용했으나 이번에 각 서버군들이 3+3 레디스 클러스터를 사용하도록 레디스를 확장
  + 레디스 클러스터는 총 3개로 각 6개(마스터3 + 복제3)로 구성되어 총 18개 노드를 운영
* 파티션 확장
  + client_message/message_aggregation 파티션의 개수를 기존 10개에서 50개로 확장
* 컨슈머 확장
  + 기존에는 클라이언트 메시지 카운터와 메시지 수집기를 각각 1개 서버에 3개 컨슈머/프로듀서를 사용하였지만 10개 서버에 5개 컨슈머/프로듀서로 확장
* transaction.timeout.ms을 기본값 60초에서 300초로 확장 (PC 리소스가 부족하다보니 컨슈머가 fence되는 현상이 있어서 리밸런스로 인해 처리량이 안나오는 현상이 있었음)

#### 초당 프로듀서 서버가 클라이언트로부터 받는 메시지 개수
기본 테스트와 동일하게 프로듀서 서버는 클라이언트로부터 초당 약 900~1000개의 메시지를 수신

#### 초당 프로듀싱 메시지 개수
![image](https://github.com/user-attachments/assets/510f175d-aa9e-4e36-8158-e11a427dc1ab)
<br/>
기본 테스트와 동일하게 프로듀서 서버가 초당 약 900개씩 메시지를 프로듀싱함
<br/>
기본 테스트와 다르게 10개의 서버에서 5개의 컨슈머/프로듀서가 작업을 분산하여 수행하므로 프로듀서 서버가 프로듀싱하는 만큼 클라이언트 메시지 카운터가 컨슈밍/프로듀싱을 수행 (약 초당 900개)

#### 초당 컨슈밍 메시지 개수
![image](https://github.com/user-attachments/assets/14b32e80-b2c5-4be7-a9e0-1f5a58b088ef)
<br/>
클라이언트 메시지 카운터가 프로듀서 서버로부터 프로듀싱하는 만큼 컨슈밍하고 프로듀싱하므로 메시지 수집기도 클라이언트 메시지 카운터만큼 컨슈밍을 수행

#### 컨슈머 그룹 랙 측정
![image](https://github.com/user-attachments/assets/b7210d38-8ef8-43d5-b96c-61755fcabb71)
<br/>
초반부에 클라이언트 메시지 카운터와 메시지 수집기의 컨슈밍 갭이 2K까지 벌어졌지만 (기본 테스트에서는 74.6K개까지 차이가 벌어짐) 시간이 흐를수록 점점 갭이 줄어들게 됨

#### 결론
레디스 클러스터 분리의 영향보다는 파티션 확장과 컨슈머 확장이 컨슈머 랙을 줄이는데 제일 큰 영향을 주는 것으로 파악
