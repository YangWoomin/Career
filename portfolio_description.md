...
#### 메시지 유실이나 적재 실패시 왜 해당 메시지부터 전부 다시 재전송하는가?
* 신뢰성있는 데이터 전송을 위해 먼저 간단하게 구현할 수 있는 Go-Back-N 방법을 도입하여 빠르게 구현해보았음
* 차후 고도화를 한다면 Selective-Repeat 방법을 사용하여 재전송이 필요한 메시지만 재전송할 수 있도록 작업할 예정 
...
* 또한 프로듀서 서버에서 클라이언트 식별을 ip:port 형식의 주소 기반으로 하고 있는데 이로써는 정확히 클라이언트를 구분할 수 없으며 클라이언트 인증이 필요함
