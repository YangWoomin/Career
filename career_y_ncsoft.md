...
* [3. DevOps](#3-DevOps)
  + [3.1 CI](#31-CI)
  + [3.2 CD](#32-CD)
* [4. Self-examination](#4-Self-examination)
...
* 다이어그램에 표시되지 않은 citadel 서버(운영 서버) 및 팀 서버 같은 커뮤니티성 서버도 있지만 미처 표현하지 못하였음
  + 로비 및 게임 서버는 커뮤니티 서버와 메시지 큐를 사용하여 통신
...
### 작업 내용
* 사용자가 외부에서 사용하는 플랫폼 계정을 게임 내 계정(account id)과 바인딩하여 외부 플랫폼 계정으로 게임을 플레이할 수 있도록 구현
  + 연동한 외부 플랫폼은 사내 계정 관리 시스템(nano)과 스팀이 있음
* 인증 서버로부터 사용자가 인증된 클라이언트에게 인증 서버 외 다른 서버나 외부 시스템에서 쉽게 인증(권한 부여)할 수 있도록 내부 토큰(LLL Token) 시스템 구현
...
> #### 개발 환경에서의 멀티 클라이언트 및 멀티 로그인을 위한 내부 계정 시스템 도입
...
>   + 계정 관리 서비스에서 인증 db에 접근하여 id/pw를 추가하거나 삭제 가능
...
> * 따라서 평문 노출을 없애고 발급한 계정의 만료 기간을 설정하기 위해 내부 토큰화 기능을 구현
...
> * 인증 서버는 기본적으로 토큰을 직접 해석하는 방법을 사용
...
> #### 개발 환경에서의 멀티 & 자동 로그인을 위한 중복 로그인 처리
> * 앞서 인증 서버에서 멀티 클라이언트와 멀티 로그인 기능의 연장선
...
<details>
<summary>World Contents Visualize 제작 배경 및 구현 과정  <span style="color:red">(제가 문제를 어떻게 해결하는지의 과정이 담겨 있습니다)</span></summary>

> [제작 배경]
> 여러 게임 서버들이 모여 하나의 게임 월드를 이루며 그 월드에는 유저 캐릭터(PC)나 NPC, 기타 상호작용하는 요소(자동차, 비행기, 무기 등)들을 모두 게임 오브젝트로 표현합니다. 이러한 데이터는  stateful 하게 게임 서버 메모리에 상주하며 실시간으로 클라이언트와 상호작용하면서 시간에 따라 생성(스폰)되고 삭제(디스폰)되거나 변경됩니다. 한두 개의 게임 오브젝트에 대한 변화는 관찰이 쉬운 편이지만 오픈 월드에서 수많은 오브젝트를 실시간으로 그리고 시각적으로 확인하기란 매우 까다로운 문제였고 당시 서버가 남기는 로그를 확인하는 게 전부여서 많은 어려움이 있었습니다. 이에 따라 시간에 따라 서버의 데이터를 확인할 수 있는 툴의 필요성이 요구되었고 게임 오브젝트들이 지리적으로 어디에 있는지, 생성과 삭제가 제대로 이뤄지는지 보여줄 툴로써 World Contents Visualizer(이하 Visualizer)가 탄생하게 되었습니다.

[시스템 설계 및 구현]
> 위의 도입 배경에서 설명한 것과 같이 게임 서버가 남겨야 할 데이터 중의 시간의 흐름에 따른 데이터 즉, 시계열 데이터를 저장할 데이터베이스가 필요했습니다. 대표적으로 InfluxDB와 Prometheus가 고려되었는데 그 중 InfluxDB를 사용하게 되었습니다. 이유로는 공식 문서가 잘 정리되어 있었고 쓰기 성능이 매우 우수하다는 점과 DB Ranking의 시계열 데이터베이스 카테고리에서 상위권인 점이 있었습니다. 그리고 가장 마음에 든 점은 InfluxQL과 Flux 등의 자체 쿼리 언어를 사용하기가 쉬웠던 점도 있습니다. pull 방식인 Prometheus와 달리 push 방식인 InfluxDB가 이 부부에서 조금 아쉬운 면이 있었지만 장점이 더 크다고 판단하여 InfluxDB를 사용하기로 결정했습니다.
> 사실 InfluxDB를 도입하는 시기가 Visualizer를 만들기 위한 시점과 동일한 것은 아니었습니다. 이 전부터 게임 서버의 CPU, Memory, FPS 등의 리소스 사용량을 모니터링하기 위해, 게임 서버가 시계열 데이터를 위의 근거에 기반해 도입한 InfluxDB에 저장하는 시스템을 구축하고 Grafana를 통해 시각화하여 조회할 수 있도록 데이터 파이프라인을 구축하였습니다. 이 데이터 파이프라인을 통계 시스템이라고 불렀습니다. 게임 서버 쪽에는 통계 서브시스템이라는 컴포넌트를 만들어서 위의 지표들을 InfluxDB에 적재하도록 모듈화했고 InfluxDB에는 bucket(브랜치 단위)과 measurement(측정 지표 카테고리 단위)로 데이터를 분류하여 저장하도록 작업했습니다. 이 통계 시스템에 Visualizer가 사용할 데이터를 추가로 전달하는 방식으로 Visualizer 데이터 적재를 쉽게 달성했습니다.
> 하지만 Grafana 같은 viewer는 맵 이미지 위에 우리가 원하는 대로 게임 오브젝트들을 보여주기에는 충분하지 못했습니다. 그래서 우리가 원하는 대로 자유롭게 보여줄 Visualizer의 실질적인 부분을 직접 만들게 되었습니다. Grafana와 같이 사용자가 웹 브라우저로 쉽게 접근할 수 있도록 웹 서비스 형태로 구축했으며 3대 웹 프론트엔드 프레임워크 중의 하나인 Vue.js를 사용하여 만들었습니다. 이에 더해 맵 뷰를 웹 브라우저에서 보여주기 위해 OpenLayers 라이브러리를 사용했습니다. 그리고 이런 프론트엔드를 위한 정적 서버로 Nginx를 두고 브라우저에서 InfluxDB API를 통해 데이터를 조회해 올 수 있도록 스크립트를 작성했습니다. (프로젝트 내에서만 사용할 인하우스 툴이라 따로 백엔드를 만들지 않았습니다)

[문제해결]
> 서버 개발자들에게 Visualizer를 서비스함으로써 서버에 자기가 만든 컨텐츠를 시간의 흐름에 따라 시각적으로 확인할 수 있도록 앞서 제작 배경에서 언급된 문제를 해결하고 서버의 상태를 보다 투명성 있게 확인할 수 있는 것으로 요구사항을 만족시켰습니다. 서버 개발자들은 마일스톤 초반에 컨텐츠를 밀어 넣고 후반에 모니터링해야 하는 시기에 적절히 잘 활용하였습니다.

[피드백 수용 1]
> Visualizer에서 출력하는 게임 오브젝트들을 필터링해서 볼 수 있도록 기능을 추가하였습니다. 많은 오브젝트가 출력되다 보니 종류별로 골라서 출력하거나 지역별로 나눠서 개수를 출력하는 기능이 필요해진 것입니다. 이 작업은 간단하게 프론트엔드 수준에서 OpenLayers 라이브러리 기능을 사용하여 어렵지 않게 처리하였습니다.

[피드백 수용 2]
> Visualizer 기능 중 하나로 게임 오브젝트들의 위치를 시간의 흐름에 따라 보여줄 수 있도록 영상처럼 보여주는 기능이 있습니다. 이 기능을 지원하기 위해 게임 서버가 데이터를 적재하는 방법이 두 가지로 나뉘게 됩니다. 하나는 snapshot 방식, 또 다른 하나는 diff 방식입니다(명칭은 제가 부여했습니다). 전자는 게임 서버가 영상 프레임 단위(일정 주기)로 매 시점의 모든 게임 오브젝트의 위치를 통계 시스템에 적재하는 것이고 후자는 프레임 단위로 위치 변화가 있는 (이동한) 오브젝트들만 변화된 위치 정보를 통계 시스템에 적재하는 것입니다. snapshot의 장점은 데이터 조회 구간이 자유롭다는 점이 있습니다. 특정 구간의 오브젝트 현황을 알고자 한다면 그 구간을 포함하는 짧은 구간을 범위로 쿼리하면 별다른 데이터 가공 없이 그대로 사용할 수 있습니다. 하지만 단점으로 게임 서버에서 통계 시스템(InfluxDB)에 데이터를 적재할 때 프레임마다 모든 게임 오브젝트의 데이터를 저장하다 보니 계산량과 전송량이 많아지고 이는 게임 서버와 네트워크에 오버헤드를 초래하게 됩니다. diff 방식의 장점은 snapshot 방식의 반대가 됩니다. 변경된 데이터만 계산하고 전송하기 때문에 게임 서버와 네트워크의 오버헤드가 적어서 좋지만 특정 구간을 조회하고자 한다면 게임 서버가 실행한 시점을 알아내야 하고 그 시작 시점부터 원하는 시점까지 프레임들을 계산해야 한다는 단점이 있습니다. 이래야 하는 이유를 간단하게 설명하자면 어떤 게임 오브젝트가 서버 실행 시점에 생성되었지만 움직이지 않거나 거의 움직이지 않는다면 조회하려는 시점에 데이터가 없을 수도 있기 때문입니다. 이러한 장단점을 고려한 결과, 게임 서버와 네트워크의 오버헤드를 감수한 snapshot 방식을 사용할 경우 나중에 정말로 오버헤드를 감당할 수 없는 지경에 도달했을 때 이 문제를 해결하기가 쉽지 않을 것으로 예상되어 diff 방식을 사용하기로 결정하였습니다. 그래서 Visualizer에서 동영상 재생 기능에 프레임을 계산하는 작업이 들어가게 되었습니다. 하지만 많은 게임 오브젝트에 대하여, 게임 서버 시작 시점이 오래되어서 긴 구간을 쿼리하고 프레임을 계산할 경우 프레임 계산 시간이 너무 오래 걸린다는 피드백을 받았습니다. 근본적인 문제를 파악하자면 게임 서버는 통계 시스템에 diff 방식으로 저장하길 원하고 Visualizer는 snapshot 방식으로 저장된 데이터를 조회하길 원하는 상황이 된 것입니다. 정말 많이 고민했습니다. 결국 diff로 저장된 데이터를 snapshot 데이터로 변환하여 저장하는 방식을 고려하게 되었고 이런 작업을 게임 서버와 Visualizer 사이에서 수행하는 별도 프로그램 툴을 제작할지 생각하였지만 운이 좋게도 InfluxDB에 Task 기능을 사용할 수 있게 되었습니다. InfluxDB는 주기적으로 InfluxDB 쿼리 언어(Flux)로 작성된 스크립트로 실행할 수 있게 Task 기능으로 지원하였습니다. 이 Task 기능을 사용하여 게임 서버가 데이터를 적재하는 현재 시점으로부터 약간 offset을 두어 주기적으로 diff 데이터를 snapshot 데이터로 변환하도록 작업하였습니다. 이 문제 해결 방법은 아주 큰 데이터를 필요할 때 한 번에 가공하기보다는 사용자가 필요로 하지 않는 유휴시간에 미리미리 부분적인 계산을 이어 나감으로써 처리하는 방법으로 볼 수 있습니다. 이로써 게임 서버는 diff 데이터를 저장하고 Visualizer는 snapshot 데이터를 조회할 수 있게 되었습니다. 기존의 > Visualizer에서 서버가 실행한 지 1~2일이 지나고 최근 5~10분 구간을 쿼리했을 때 프레임 계산에 몇 시간이 걸렸었는데 이 작업 이후에 5~10분 구간 쿼리만 고려하게 되어 1분 미만으로 시간이 단축되었습니다.

[최후의 최후까지]
> diff 데이터에서 snapshot 데이터로의 변환이 부분적으로 그리고 자동으로 된다고 하여서 모든 것들이 끝난 것은 아닙니다. 아직 잠재적인 문제가 있었는데 게임 오브젝트들이 무수히 많아질 경우 InfluxDB의 Task가 뒤로 밀릴 수도 있는 것입니다. 이런 문제가 당장에 발생하진 않았지만 컨텐츠가 많아지다 보면 충분히 발생할 수 있는 현상으로 볼 수 있습니다. 이럴 경우를 대비해 고민한 것으로, 일차적으로 게임 서버가 위치 정보를 저장할 때 불필요한 게임 오브젝트 수를 최대한 줄여보는 것과 이차적으로 테이블(measurement) 단위로 오브젝트 종류별로 분산 저장하고 이를 처리하는 Task들을 병행으로 돌리는 것입니다. 이 방법은 흔히 RDBMS에서 사용하는 파티셔닝에 가깝고 가능하다면 샤딩까지 고려할 수도 있겠습니다. 
...
* 컨텐츠적인 로그도 따로 파싱 및 분류하여 통계 산출 지원
...
* 로그 저장소에 저장된 로그들을 다양한 방식으로 조회 및 통계 산출 작업이 가능
### Trouble shooting
<details>
<summary>Trouble shooting 사례 보기</summary>
...
>   + 이 때 노드에 저장된 샤드 개수가 max_shards_per_node 설정을 초과할 경우 에러를 출력하고 적재되지 않음
...
### Build & Run
* 크로스 플랫폼(Windows/Linux)을 위해 CMake/Make 사용
...
* 서버 실행 환경으로 크로스 플랫폼을 위해 윈도우 컨테이너로 실행할 수 있는 도커 이미지 생성
...
> * 2023 GStar 출시 준비중에 리눅스 환경에서의 빌드와 실행은 중단되었고 (지스타 준비에 올인하기 위해) 이쯤 전 TD가 퇴사하면서 리눅스 환경은 지원하지 않도록 (암묵적으로) 결정
...
> * K8s 클러스터 구축 시 CNI 플러그인 도입은 필수 (CNI, Container Network Interface)
...
> * Calico CNI 플러그인은 encapsulated/unencapsulated 모델 모두를 지원 (기본은 unencapsulated)
...
<details>
<summary>네트워크 보안 문제로 인한 K8s On-Premise 사용 중지 이슈 정리</summary>

> * K8s On-Premise 사용 중에 서버 pod간 연결이 간헐적으로 단절되는 현상이 있었음
> * K8s 클러스터 자체에서 발생하는 문제가 아닌 한쪽 endpoint에서 강제로 연결이 종료되는 현상
>   + 그렇다고 해서 프로그래밍적인 문제도 아니었음 (K8s 클러스터 환경에서만 발생함)
> * K8s 클러스터 노드 중에 리눅스 노드들이 있는 호스트 네트워크 대역과 윈도우 노드들이 있는 호스트 네트워크 대역이 달랐고 이 둘 사이에서만 연결이 끊어지는 것을 파악
> * 네트워크 보안 부서와 논의한 결과 K8s Calico CNI 플러그인에 대한 설명을 대략적으로 듣고서 회사에서 제공하지 않는 별도 네트워크를 구축하여 사용하는 것은 보안상 차단될 수 있다고 전달 받음
>   + 아마 IP-in-IP 기술에 대한 검열 때문인 것으로 추측 (당시 이 기술에 대한 언급은 없었음)
> * 어차피 NCKUBE 2.0에서 K8s 혼합 클러스터가 지원될 예정이라는 소식을 접했어서 추후 NCKUBE 2.0으로 갈아 타기로 결정
</details>

* NCKUBE 2.0에 서버 배포까지 성공하였으나 클러스터 외부로 노출하는 포트에 대해 반복적인 연결 수립/해제가 발생하여 NCKUBE 2.0으로 migration 실패
...
> * K8s On-Premise 도입시 K8s의 윈도우 노드는 Process Isolation 모드만이 컨테이너(파드)를 실행할 수 있음을 알게 되었고 K8s 클러스터 뿐만 아니라 Docker Standalone 실행 시에도 Process Isolation 모드(traditional process isolation mode same as linux container)를 사용하도록 변경
...
> * 위 그림에서 알 수 있다시피 Process Isolation 모드는 각 컨테이너마다 전용 OS가 없고 호스트 OS를 공유하기 때문에 자원을 좀 더 효율적으로 사용하여 더 좋은 퍼포먼스를 낼 수 있음
